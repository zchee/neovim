...
describe('mimalloc integration', function()
  itp('nvim_mem_init() is idempotent', function()
    cimp.nvim_mem_init()
    cimp.nvim_mem_init()
  end)

  itp('xmalloc() ownership matches build setting', function()
    local enabled = cimp.nvim_mimalloc_enabled()
    local p = cimp.xmalloc(32)
    assert.is_true(p ~= ffi.NULL)
    eq(enabled, cimp.nvim_mimalloc_check_owned(p))
    cimp.xfree(p)
  end)
end)
